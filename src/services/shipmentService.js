...
// Create shipment
const createShipment = async (shipmentData, userId, companyId) => {
  try {
    ...
    const shipmentNumber = await generateShipmentNumber(companyId);
...
const getShipmentByOrderId = async (orderId, companyId) => {
  try {
    const shipment = await prisma.shipment.findFirst({
      where: {
        orderId,
...
// Update shipment
const updateShipment = async (shipmentId, updateData, userId, companyId) => {
  try {
...
const getShipments = async (companyId, page = 1, limit = 10, filters = {}) => {
  try {
...
// Delete shipment
const deleteShipment = async (shipmentId, companyId) => {
  try {
    // Check if shipment exists and belongs to company
    const existingShipment = await prisma.shipment.findFirst({
      where: {
