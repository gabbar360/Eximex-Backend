...
class PurchaseOrderService {
  ...
  // Calculate totals
  calculateTotals(items, cgstRate = 6, sgstRate = 6) {
  ...
  async createPurchaseOrder(data, userId, companyId) {
    console.log('Incoming request data:', JSON.stringify(data, null, 2)); // Debug log
    const { items, ...poData } = data;

    // Generate PO number if not provided
    if (!poData.poNumber) {
      poData.poNumber = await this.generatePoNumber(companyId);
    }
  ...
  // Get Purchase Orders
  async getPurchaseOrders(companyId, filters = {}, dataFilters = {}) {
  ...
  async getPurchaseOrderById(id, companyId) {
  ...
  async updatePurchaseOrder(id, data, userId, companyId) {
    const existingPo = await this.getPurchaseOrderById(id, companyId);
  ...
  async deletePurchaseOrder(id, companyId) {
    const purchaseOrder = await this.getPurchaseOrderById(id, companyId);
  ...
  async generatePurchaseOrderPDF(id, companyId, logoBase64 = null) {
    const purchaseOrder = await this.getPurchaseOrderById(id, companyId);
