...
export const setUserPermissions = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, null, 'User permissions updated successfully')
  );
});
...
export const getUserSidebarMenu = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, menu, 'User sidebar menu fetched successfully')
  );
});

export const getUserWithPermissions = asyncHandler(async (req, res) => {
  const { userId } = req.params;
  const userWithPermissions = await userPermissionService.getUserWithPermissions(parseInt(userId));
  return res.status(200).json(
    new ApiResponse(200, userWithPermissions, 'User with permissions fetched successfully')
  );
});
